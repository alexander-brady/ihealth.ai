...
def analyze_audio_exposure(file_path, output_directory):
    ...
    # Get key stats
    total_exposure = audio_data['value'].sum()
    average_exposure = audio_data['value'].mean()
    min_exposure = audio_data['value'].min()
    max_exposure = audio_data['value'].max()

    # Group data by day and sum the audio exposure for each day
    daily_exposure = audio_data.groupby(audio_data['startDate'].dt.date)['value'].sum()

    # Convert the index (dates) to strings for JSON serialization
    daily_exposure = daily_exposure.rename(index=str)

    # Find the highest and lowest exposure day
    highest_exposure_day = daily_exposure.idxmax()
    highest_exposure_value = daily_exposure.max()
    
    lowest_exposure_day = daily_exposure.idxmin()
    lowest_exposure_value = daily_exposure.min()

    # Create a dictionary to hold the analysis
    audio_analysis = {
        "total_audio_exposure": total_exposure,
        "average_exposure_per_record": average_exposure,
        "min_exposure_per_record": min_exposure,
        "max_exposure_per_record": max_exposure,
        "daily_audio_exposure_summary": daily_exposure.to_dict(),
    ...
    breakdown_comment = f"Highest Audio Exposure Day: {highest_exposure_day} with {highest_exposure_value:.2f} dB. This suggests prolonged or high-volume listening sessions."
    breakdown_comment += f"\nLowest Audio Exposure Day: {lowest_exposure_day} with {lowest_exposure_value:.2f} dB, possibly reflecting a day of less or no headphone use."
    
    # Weekday vs Weekend analysis
    audio_data['weekday'] = audio_data['startDate'].dt.weekday
    weekday_exposure = audio_data[audio_data['weekday'] < 5]['value'].sum()
    weekend_exposure = audio_data[audio_data['weekday'] >= 5]['value'].sum()

    if weekday_exposure > weekend_exposure:
        breakdown_comment += f"\nYou tend to use your headphones more on weekdays with {weekday_exposure:.2f} dB exposure compared to {weekend_exposure:.2f} dB on weekends."
    else:
        breakdown_comment += f"\nYou tend to use your headphones more on weekends with {weekend_exposure:.2f} dB exposure compared to {weekday_exposure:.2f} dB on weekdays."
    ...
    print(f"Total Audio Exposure: {total_exposure:.2f} dB")
